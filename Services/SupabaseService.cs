    ...
    public class SupabaseService
    {
        ...
        public async Task<UserModel> GetCurrentUserProfileAsync()
        {
            try
            {
                ...
                var response = await _client.From<UserModel>().Where(u => u.Id == userId).Single();
                return response;
            }
            catch (Exception ex)
            {
        ...
        public async Task<(bool IsSuccess, string Message)> CreatePriorityRequestAsync(string reason)
        {
            ...
            {
                Console.WriteLine($"Request Creation Error: {ex.Message}");
        ...
        public async Task<List<RequestModel>> GetRiderHistoryAsync()
        {
            ...
            {
                Console.WriteLine($"Rider History Error: {ex.Message}");
        ...
        public async Task<bool> DeleteRequestAsync(string requestId)
        {
            ...
            {
                Console.WriteLine($"Delete Request Error: {ex.Message}");
                return false;
            }
        }
        ...
        public async Task<List<DriverRequestDisplayModel>> GetPendingRequestsAsync()
        {
            ...
            {
                Console.WriteLine($"Fetch Pending Requests Error: {ex.Message}");
                return displayList;
            }
        }

        // Fetch the logged-in driver's completed history
        ...
        // Mark a single request as completed and assign the driver
        public async Task<bool> CompleteRequestAsync(string requestId)
        {
            try
            {
                ...
                await _client.From<RequestModel>()
                             ...
                             .Set(x => x.DriverId, currentUser.Id)
                             .Update();
            ...
            {
                Console.WriteLine($"Complete Single Request Error: {ex.Message}");
                return false;
            }
        }

        // Bulk complete all pending requests
        public async Task MarkAllPendingAsCompleteAsync()
        {
            try
            {
                var driverId = _client.Auth.CurrentUser?.Id;
                if (driverId == null) return;

                var pending = await _client.From<RequestModel>().Where(r => r.Status == "pending").Get();

                foreach (var req in pending.Models)
                {
                    await _client.From<RequestModel>()
                                 .Where(x => x.Id == req.Id)
                                 .Set(x => x.Status, "completed")
                                 .Set(x => x.DriverId, driverId)
                                 .Update();
                }
            }
            catch (Exception ex)
        ...
        public async Task<List<UserModel>> GetAllUsersAsync()
        {
            ...
            {
                Console.WriteLine($"Admin User Fetch Error: {ex.Message}");
